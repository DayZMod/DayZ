...
dz_fn_switchWeapon =
{
	...
	switch _this do
	{
		...
		case 0:
		{
			...
			[[findDisplay 106], "onLBSelChanged"] call dz_fn_ui_gear_handleGear;
		};
		...
		case 2:
		{
			...
			if IS_PRIMARY(_current) exitWith
			{
				...
				if (IS_MELEE(_current) && { !IS_MELEE(Player_CarryWeapon) }) then
				{
					if (dayz_quickSwitch) then
						{ true call dz_fn_switchWeapon_swap }
					else
...
dz_fn_switchWeapon_find =
{
	...
	if (Player_CarryWeapon != "" && { _this == MELEE(Player_CarryWeapon) })
		exitWith { 2 };
...
dz_fn_switchWeapon_swap =
{
	...
	else
	{
		...
		[
			primaryWeapon player,
			player removeWeapon primaryWeapon player,
			player addWeapon Player_CarryWeapon
		] select 0;
	};
