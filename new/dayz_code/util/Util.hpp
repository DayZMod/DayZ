	...
	/* Waits until the condition is met, or <timeout> seconds have passed.
	Returns true if the condition was met, false if timed out.
	
	Example:
		Util_WaitUntil({!isNil "myGlobalVariable"}, 30)
	*/
	#define Util_WaitUntil(predicate, timeout) ([predicate, timeout, _this] call dz_fn_util_waitUntil)
	
	/* Sleeps for at least one frame. */
	#define Util_SleepFrame() (call dz_fn_util_sleepFrame)
