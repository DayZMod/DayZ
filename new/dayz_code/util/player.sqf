...
dz_fn_player_addItem =
{
	...
	switch (_this select 0) do
	{
		case 0:
		{
			...
			{
				_num = 0;
				{ _num = _num + _x } count (getWeaponCargo _veh select 1);
		...
		case 1:
		{
			...
			{
				_num = 0;
				{ _num = _num + _x } count (getMagazineCargo _veh select 1);
...
dz_player_stanceAnims = ["Perc", "Pknl", "Ppne"];
dz_player_weaponAnims = ["Wlnr", "Wnon", "Wpst", "Wrfl"];