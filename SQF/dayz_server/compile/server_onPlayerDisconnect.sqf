...
diag_log format["INFO - Player: %3(UID:%1/CID:%2) as (%4), Disconnecting %5", 
	...
	typeOf _playerObj, 
	if ((_lastDamage > 5 AND (_lastDamage < 30)) AND ((alive _playerObj) AND (_playerObj distance respawn_west_original >= 2000))) then {" while in combat ("+str(_lastDamage)+" seconds left)"} else {""}
];
*/ 
...
if (_characterID != "?") then {
	...
	[_playerUID,_characterID,3,(_playerObj call fa_plr2str),((getPosATL _playerObj) call fa_coor2str),if ((_lastDamage > 5 AND (_lastDamage < 30)) AND ((alive _playerObj) AND (_playerObj distance respawn_west_original >= 2000))) then {" while in combat ("+str(_lastDamage)+" seconds left)"} else {""}] call dayz_recordLogin;
